...
module.exports = {
    ...
    async execute({interaction,message,args}){
        ...
        let userdata = await userdb.get(`${message.guild.id}/${self.user.id}`);
        ...
        if(userdata == userdata2){
            console.log(`Duplicated userdata found at [${message.guild.id}/${self.user.id}] and [${message.guild.id}/${member.user.id}].`);
            functions.resetLog(message.guild.id,self.user.id,member.user.id,userdata,userdata2);
            await userdb.set(`${message.guild.id}/${self.user.id}`, new Data('user',{}));
            await userdb.set(`${message.guild.id}/${member.user.id}`, new Data('user',{}));
        ...
        await userdb.set(`${message.guild.id}/${self.user.id}`, userdata);
